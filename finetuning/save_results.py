...
train_zs = load_from_disk(os.path.join('train_zs11_prompt_dataset'))
test_sft = load_from_disk('/data/yguo/dataset/test_zs11_prompt_dataset') 
...
for i in range(len(prompt_df)): 
    ...
    texts = [row['prompt']]*prompt_number
    ...
    for j in range(prompt_number): 
        ...
        generated_texts[j] = generated_texts[j].split('\n', 1)[0]    
        while generated_texts[j].startswith('"') and generated_texts[j].endswith('"'):
            generated_texts[j] = generated_texts[j][1:-1]
        prompt_df.iloc[i, j+3] = generated_texts[j] # Only keep the caption and discard the explanation.
        
    # for j in range(prompt_number): 
    #     result = llm(row['prompt']5)
    #     prompt_df.iloc[i, j+3] = result

prompt_df.to_csv('zs_gpto11_results_50gen.csv', index=False)
################ Save SFT Result ################
